...
public class ServerController {
    ...
    private void loadChatHistory(String user1, String user2, String tableName) {
        MessageStack messageStack = ChatHistoryFetcher.fetchChatHistory(user1, user2,tableName);
    ...
    public void handleSendButtonAction() {
        ...
        try {
            login2Controller l1 = new login2Controller();
            login1Controller l2 = new login1Controller();
            if (l1.dos != null) {
                l1.dos.writeUTF(out);
                l1.dos.flush();
    ...
    public void startMessageReceiver() {
        ...
        new Thread(() -> {
            System.out.println("Message receiver thread started for server");
            try {
                while (true) {
    ...
    public void generateTable() throws Exception {
        ...
        if(found1) {
            loadChatHistory(phone2, phone1,tableName1);
        }
        else if(found2) {
            loadChatHistory(phone2, phone1,tableName2);
        }
        else if (found2 == false) {
            String tableName = phone1+"_WITH_"+ phone2;
            String table = "CREATE TABLE "+tableName+" (id INT AUTO_INCREMENT PRIMARY KEY,sender VARCHAR(255) NOT NULL,receiver VARCHAR(255) NOT NULL,message TEXT NOT NULL,timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP)";

            PreparedStatement pst = con.prepareStatement(table);
            pst.executeUpdate();

            loadChatHistory(phone1, phone2,tableName);
